...
StepText = uicontrol('Style','Text','String','Time Step(sec):','Position',[10,410,100,20],'HorizontalAlignment','left','FontSize',9);
...
TimeText = uicontrol('Style','Text','String','Run Time(sec):','Position',[10,380,100,20],'HorizontalAlignment','left','FontSize',9);
...
uicontrol('Style','Text','String','sec','Position',[210,20,40,20],'HorizontalAlignment','left','FontSize',9);
...
switch pbcase
    case 'on'
        ...
        figure()
        xyhist = (0:0.01:max(sqrt(Vel(:,1).^2+Vel(:,2).^2)));
        [anzahlhist,x] = hist(sqrt(Vel(:,1).*Vel(:,1) + Vel(:,2).*Vel(:,2)), xyhist);
        anzahlhist=anzahlhist./(sum(anzahlhist)*mean(diff(xyhist)));
        %anzahlhist=anzahlhist./trapz(x,anzahlhist);
        maxwell=@(b,v) b(1).*v.*exp(-b(1)*v.^2);
        b=[1e-3];
        [a,b,c,d,e]=nlinfit(xyhist,anzahlhist,maxwell,b);
        xrange=linspace(min(xyhist),max(xyhist),100);
        hold on;
        scatter(xyhist,anzahlhist);
        plot(xrange, maxwell(a,xrange),'r');
        %xlabel('v',16);
        %ylabel ('f(v)',16);
               
        xlabel('Velocity Magnitude','fontsize',16);
        ylabel('Probability density','fontsize',16);
        legend(['Probability density distribution at time ',num2str(time)]);
        hold off;
