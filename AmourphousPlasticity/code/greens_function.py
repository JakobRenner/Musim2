...
def get_inclusion_stress(K,x,y):
    ...
    # this function returns the stress field as a 2D matrix
...
def get_inclusion_stress_1D(K,i):
    
    # this function returns the stress field as a 1D vector    
    
     L = int(K.shape[0]/2)   
     x = np.unravel_index(i,(L,L))
     
     return np.ravel(get_inclusion_stress(K,x[0],x[1]))
