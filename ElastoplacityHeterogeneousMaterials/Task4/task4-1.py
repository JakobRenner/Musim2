...
def main():
	...
	#
	# - 'outfile' should now show up in the first line of "...append(..) !"
	# - dv is the vertical displacement of the top nodes
	oofem.write_LinElast_inputfile(lx,ly,nodes_x,nodes_y,dv, infile, outfile)
	#oofem.write_J2plastic_inputfile(lx,ly,nodes_x,nodes_y,dv, infile, outfile)
	# ==================================================================
	...
	sim = oofem.readOutputfile(outfile, True)
	# ------------------------------------------------------------------
	...
	for ney in range(nodes_y-1):
		for nex in range(nodes_x-1):
			n=nex+ney*(nodes_x-1)
			eps[ney,nex] = sim.step[-1].element[n].avstrain.yy
